...
def test_strexd_t1(thumb_v6_without_fetch):
    arm = thumb_v6_without_fetch
    arm.opcode = 0b11101000110000100001001101110000
    arm.opcode_len = 32
    ...
    arm.registers.set(opcode.t2, 0xAABBCCDD)
    arm.emulate_cycle()
    assert arm.registers.get(opcode.d) == 1
    assert ram_memory[0, 8] == b'\x00\x00\x00\x00\x00\x00\x00\x00'


def test_strexd_a1(arm_v6_without_fetch):
    arm = arm_v6_without_fetch
    arm.opcode = 0b11100001101000000011111110010100
    arm.opcode_len = 32
    # setting Data Region registers
    arm.registers.drsrs[0].en = 1  # enabling memory region
    arm.registers.drsrs[0].rsize = 0b00010  # setting region size
    ...
    opcode = arm.decode_instruction(arm.opcode)
    opcode = opcode.from_bitarray(arm.opcode, arm)
    assert isinstance(opcode, StrexdA1)
    assert opcode.d == 3
    assert opcode.n == 0
    assert opcode.t == 4
    assert opcode.t2 == 5
