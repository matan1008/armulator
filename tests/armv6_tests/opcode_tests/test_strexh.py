...
def test_strexh_t1(thumb_v6_without_fetch):
    arm = thumb_v6_without_fetch
    arm.opcode = 0b11101000110000100001000001010000
    arm.opcode_len = 32
...
def test_strexh_a1(arm_v6_without_fetch):
    arm = arm_v6_without_fetch
    arm.opcode = 0b11100001111000000010111110010001
    arm.opcode_len = 32
    # setting Data Region registers
    arm.registers.drsrs[0].en = 1  # enabling memory region
    arm.registers.drsrs[0].rsize = 0b00010  # setting region size
    ...
    opcode = arm.decode_instruction(arm.opcode)
    opcode = opcode.from_bitarray(arm.opcode, arm)
    assert isinstance(opcode, StrexhA1)
    assert opcode.d == 2
    assert opcode.n == 0
    assert opcode.t == 1
    arm.registers.set(opcode.n, 0x0F000000)
