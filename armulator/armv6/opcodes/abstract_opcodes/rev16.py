...
class Rev16(Opcode):
    ...
    def execute(self, processor):
        if processor.condition_passed():
            ...
            result = set_substring(result, 31, 24, substring(m, 23, 16))
            result = set_substring(result, 23, 16, substring(m, 31, 24))
            result = set_substring(result, 15, 8, substring(m, 7, 0))
